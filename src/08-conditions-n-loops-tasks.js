...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${isStartIncluded ? '[' : '('}${f}, ${s}${isEndIncluded ? ']' : ')'}`;
}
 ...
 */
function isCreditCardNumber(ccnn) {
  let ccn = ccnn;
  ccn = ccn.toString().split('');
  let s = 0;
  for (let i = 0; i < ccn.length - 1; i += 1) {
    if (i % 2 === 0) {
      ccn[i] *= 2;
      ccn[i] = ccn[i].toString().split('');
    }
    // ccn[i].toString().split('').forEach((e) => s += Number(e));
    // } else {
    //   s += Number(ccn[i]);
    // }
  }
...
function getDigitalRoot(num) {
  ...
  if (s > 9) { getDigitalRoot(Number(s)); }
  return s;
}
